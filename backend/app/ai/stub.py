...
class StubAIProvider:
    def generate(self, prompt: str, context: Subgraph, budget_tokens: int) -> str:
        ...
        # A plausible minimal PHPUnit test class (not just a comment): the renderer's
        # Loop-0 self-repair (ADR-0070) retries anything that isn't a test class, so the
        # stub must look like real AI output. The marker/fingerprint/budget are kept in
        # comments (assertions + determinism depend on them).
        return (
            "<?php\n\n"
            ...
            "use Tests\\TestCase;\n\n"
            f"class Stub{fingerprint}Test extends TestCase\n"
            "{\n"
            f"    public function test_stub_{fingerprint}(): void\n"
            "    {\n"
